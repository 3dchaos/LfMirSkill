...
- For shared feature modules that are called by many NPC scripts through `#CALL [系统功能\xxx.txt] @label`, this project may require a wrapped call-file layout: put a `{` immediately under the first label header and put the matching `}` at the very end of the file. Preserve that wrapper when editing or regenerating the module, and keep NPC files as thin menu entries that only dispatch to the shared labels.
